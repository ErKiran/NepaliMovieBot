...
class NepaliImdbCrawler:
    ...
    def scrape_page(self, url):
        ...
        nexturl = '' if not nextdiv else urljoin(
            self.imdb_url, nextdiv['href'])
        return data, nexturl
    ...
    def _get_single_movie(self, div):
        ...
        year = title_div.find(
            'span', {'class': 'lister-item-year text-muted unbold'}).text.strip()
        if year:
        ...
        info_map['rating'] = None if not rating_div else float(
            rating_div.text.strip())

        vote_div = div.find('p', {'class': 'sort-num_votes-visible'})
        info_map['votes'] = None if not vote_div else int(
            re.findall(r'\d+', vote_div.text.strip())[0])
